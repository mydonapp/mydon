...
describe('AccountsService', () => {
  ...
  describe('mapAccountsToGrouped', () => {
    it('should filter and map accounts by type with currency conversion', async () => {
      ...
      const result = await service.mapAccountsToGrouped(mockAccounts as Account[], AccountType.ASSETS);
      ...
      expect(mockForexService.convertCurrency).toHaveBeenCalledWith(800, 'EUR', 'CHF', expect.any(Date));
    });
    ...
    it('should return empty array when no accounts match the type', async () => {
      ...
      const result = await service.mapAccountsToGrouped(mockAccounts as Account[], AccountType.INCOME);
