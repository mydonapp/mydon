...
export class AddOrganizationsAndMemberships1778934643756 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "organizations" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" character varying NOT NULL, "kind" "public"."organizations_kind_enum" NOT NULL DEFAULT 'PERSONAL', "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_6b031fcd0863e3f6b44230163f9" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."organization_memberships_role_enum" AS ENUM('OWNER', 'ADMIN', 'MEMBER', 'ACCOUNTANT')`);
        await queryRunner.query(`CREATE TABLE "organization_memberships" ("organization_id" uuid NOT NULL, "user_id" uuid NOT NULL, "role" "public"."organization_memberships_role_enum" NOT NULL DEFAULT 'OWNER', "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_caa73db1b161fa6b3a042290fe7" PRIMARY KEY ("organization_id", "user_id"))`);
        ...
        await queryRunner.query(`ALTER TABLE "organization_memberships" ADD CONSTRAINT "FK_5352fc550034d507d6c76dd2901" FOREIGN KEY ("user_id") REFERENCES "user"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
