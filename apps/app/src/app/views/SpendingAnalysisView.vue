  ...
  <div class="min-h-screen bg-primary">
    ...
    <div class="container mx-auto px-4 sm:px-6 py-6 sm:py-8 max-w-none xl:max-w-screen-2xl">
      ...
      >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <div class="card bg-gradient-to-br from-info/10 to-info/5 border border-info/20 shadow-lg">
            <div class="p-4">
              ...
              >
                {{ spendingChange >= 0 ? '+' : '' }}{{ formatCurrency(spendingChange, 'CHF') }}
              </p>
            </div>
          </div>
          ...
          <div class="card bg-gradient-to-br from-accent/10 to-accent/5 border border-accent/20 shadow-lg">
            <div class="p-4">
              <div class="flex items-center justify-between mb-2">
                ...
                <RiPercentLine
                  class="w-5 h-5"
                  :class="spendingChangePercentage >= 0 ? 'text-error' : 'text-success'"
                />
              </div>
              <p
                class="text-2xl font-bold"
                :class="spendingChangePercentage >= 0 ? 'text-error' : 'text-success'"
              >
                {{ spendingChangePercentage >= 0 ? '+' : '' }}{{ Math.abs(spendingChangePercentage).toFixed(1) }}%
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div class="card bg-secondary shadow p-1.5 sm:p-2 rounded-lg">
            <div class="p-4">
              ...
              <div class="h-64 bg-primary rounded-lg flex items-center justify-center">
                <div
                  v-if="historicalSpending.length === 0"
                  class="text-center"
                >
                  <RiLineChartLine class="w-12 h-12 mx-auto text-gray-400 mb-4" />
                ...
                >
                  ...
                  <div class="flex items-end justify-between h-full p-4 space-x-2">
                    <div
                    ...
                    >
                      <div
                        ...
                        :style="{
                          height: `${(Math.abs(period.amount) / maxSpending) * 100}%`,
                        }"
                        :title="formatCurrency(period.amount, 'CHF')"
                      ></div>
                      <span class="text-xs mt-2 text-center">{{ period.label }}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                >
                  <div class="flex items-center flex-1 min-w-0">
                    <div class="w-3 h-3 bg-error rounded-full mr-3 flex-shrink-0"></div>
                    <div class="flex-1 min-w-0">
                      <h4 class="font-semibold text-sm truncate">
                        {{ account.name }}
                      </h4>
                      <p class="text-xs text-gray-400">
                        {{ ((Math.abs(account.amount) / totalCurrentPeriodSpending) * 100).toFixed(1) }}%
                      </p>
                    </div>
                  </div>
        ...
        <div class="card bg-gradient-to-r from-warning/10 to-warning/5 border border-warning/20 shadow-lg">
          <div class="p-4">
            <div class="flex items-center justify-between mb-4">
              <h2 class="text-xl font-bold text-white text-lg sm:text-xl text-warning">
                {{ t('views.spendingAnalysis.budgetComparison') }}
              </h2>
              <div class="bg-warning text-black px-2 py-1 rounded text-sm font-semibold">
                {{ t('views.spendingAnalysis.comingSoon') }}
              </div>
            </div>
            <div class="text-center py-8">
              <RiInformationLine class="w-16 h-16 mx-auto text-warning/60 mb-4" />
...
const expenseAccounts = computed(() => accounts?.value?.expense?.accounts || []);
...
const totalCurrentPeriodSpending = computed(() => {
  return currentPeriodAccountBreakdown.value.reduce((sum: number, account: any) => sum + Math.abs(account.amount), 0);
});
