...
**Interactive back-swipe + animated back button (mobile):** `PageStageComponent` wraps `<router-outlet>` and gives the back navigation an iOS-style slide. On every `NavigationStart` it deep-clones the outgoing page's DOM (+ scrollY) onto a small stack, settling the snapshot ~800 ms after `NavigationEnd` so the back preview shows real content, not its skeleton. A fixed left-edge `pan-y` strip captures the pointer and drives the slide: the live page is translated right via `--swipe-x` while the previous-page snapshot parallaxes in from `-25%` with a dim wash, the `.page-prev` layer inset by `env(safe-area-inset-top)` to align with the real content. Release past `40 %` (or a fast flick) commits — animate out then `Location.back()`; `NavigationEnd` tears the clone down. Detail-page back chevrons replay the same animation via a tiny `BackNavigator` service that PageStage registers a handler with — falls back to the plain `routerLink` on desktop. **Every UI state that affects the template is a signal** (`armed`, `swiping`, `finishing`, `swipeX`, `swipeP`, stack length); volatile getter bindings caused `NG0100: ExpressionChangedAfterItHasBeenChecked` under zoneless CD and disrupted the page reattach. Gate on `(max-width: 767px)` + `(pointer: coarse)` + not `reduce`; the fixed bottom nav is outside the stage so it never moves.
